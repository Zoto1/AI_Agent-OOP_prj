...
struct TaskResult {
    ...
    std::optional<std::string> error; // có giá trị nếu task bị lỗi khi chạy (không chấm điểm được)
};
...
// - Chỉ NỐI các layer lại: Environment -> AgentLoop -> Trajectory -> Evaluator -> file JSON
// Đúng nguyên tắc "AgentLoop KHÔNG biết Harness tồn tại" (mục 4.4 đề bài):
// HarnessRunner tiêm (inject) step_hook vào AgentLoop từ bên ngoài (Observer/Hook pattern),
// AgentLoop chỉ gọi hook đó ra mà không biết ai đang lắng nghe.
class HarnessRunner {
private:
    ...
    std::string workspace_root; // nơi mỗi task có 1 thư mục làm việc riêng (NativeEnvironment)
    double success_threshold;   // score >= ngưỡng này thì coi task là "success"

    std::vector<TaskDefinition> loadTasks(const std::string& tasks_json_path) const;
    std::shared_ptr<Evaluator> makeEvaluator(const TaskDefinition& task) const;
    std::shared_ptr<Environment> makeEnvironment(const TaskDefinition& task) const;
...
public:
    ...
    // In báo cáo tổng hợp: success rate tổng, breakdown theo eval_type (mục 7.3/VIII đề bài)
