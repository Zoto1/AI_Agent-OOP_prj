...
MemoryTool::MemoryTool(const std::string& path) 
    : Tool("memory", "Luu va lay ngu canh bo nho nguoi dung"), storage_path(path) {}

// Hàm execute nhận lệnh từ registry tool
std::string MemoryTool::execute(const std::map<std::string, std::string>& args) {
    ...
    if (action == "load") {
        ...
        return load_context(queryIt->second);
    }

    return "Loi: Hanh dong '" + action + "' khong hop le!";
}
bool MemoryTool::save_context(const std::string& key, const std::string& value) {
    std::cout << "[Memory] Dang luu: " << key << " -> " << value << " vao " << storage_path << "\n";
    return true; 
}
std::optional<std::string> MemoryTool::load_context(const std::string& query) {
    bool tim_thay = false;
...
void MemoryTool::init() {
    auto instance = std::make_shared<MemoryTool>();
